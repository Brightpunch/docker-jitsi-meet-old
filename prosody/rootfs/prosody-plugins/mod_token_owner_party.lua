...
module:hook("muc-occupant-pre-join", function (event)
    ...
    if context_user then
        if context_user["affiliation"] == "owner" or
           ...
           module:log(LOGLEVEL, "moderator joined")
           return
        end
    end
    ...
    if occupant_count < 2 then
        module:log(LOGLEVEL, "conference has not started yet")
...
module:hook("muc-occupant-left", function (event)
    ...
    if room:get_affiliation(occupant.jid) ~= "owner" then
        module:log(LOGLEVEL, "a participant leaves, %s", occupant.jid)
        return
    end
    ...
    module:log(LOGLEVEL, "an owner leaves, %s", occupant.jid)
    ...
    timer.add_task(TIMEOUT, function()
        ...
        module:log(LOGLEVEL, "conference finished")
    end)
end)
